...
with raw_disputes as (
  select
    json_extract_scalar(_airbyte_data, '$.id') as dispute_id,
    ...
    _airbyte_loaded_at
  from {{ source('airbyte_raw', 'airbyte_raw_disputes') }}
),
...
deduped as (
  select
    ...
    updated_at,
    _airbyte_loaded_at,
    row_number() over (partition by dispute_id order by _airbyte_loaded_at desc) as rn
  from raw_disputes
)
